...
static char * reiserfs_cpu_offset (struct cpu_key * key)
{
  if (cpu_key_k_type(key) == TYPE_DIRENTRY)
    sprintf (off_buf, "%Lu(%Lu)", 
	     (__u64 )GET_HASH_VALUE (cpu_key_k_offset (key)),
	     (__u64 )GET_GENERATION_NUMBER (cpu_key_k_offset (key)));
  else
    sprintf (off_buf, "0x%I64x", (__u64)cpu_key_k_offset (key));
  return off_buf;
}
...
static char * le_offset (struct key * key)
{
  ...
  if (le_key_k_type (version, key) == TYPE_DIRENTRY)
    sprintf (off_buf, "%Lu(%Lu)", 
	     (__u64)GET_HASH_VALUE (le_key_k_offset (version, key)),
	     (__u64)GET_GENERATION_NUMBER (le_key_k_offset (version, key)));
  else
    sprintf (off_buf, "0x%Lx", (__u64)le_key_k_offset (version, key));
  return off_buf;
}
...
static void sprintf_buffer_head (char * buf, struct buffer_head * bh) 
{
  sprintf (buf, "dev %s, size %d, blocknr %ld, count %d, list %d, state 0x%lx, page %p, (%s, %s, %s)",
	   kdevname (bh->b_dev), bh->b_size, bh->b_blocknr, atomic_read (&(bh->b_count)), 0,
