...
int FASTCALL B_IS_IN_TREE (const struct buffer_head * p_s_bh)
{
...
static char  prepare_for_delete_or_cut(
				       ...
				       int                 * p_n_cut_size,
				       __u64    n_new_file_length /* MAX_KEY_OFFSET in case of delete. */
    ) {
				 ...
				 struct key * key)
{
	...
	if (retval != ITEM_FOUND) {
	    ...
	    if ( !( (__u64) GET_HASH_VALUE (le_key_k_offset (le_key_version (key), key)) == 0 && \
