...
*/ 
int FASTCALL buffer_journal_new(const struct buffer_head *bh) {
  if (bh) 
...
static void submit_logged_buffer(struct buffer_head *bh) {
    ...
    get_bh(bh) ;
    bh->b_flags |= B_CALL;
    bh->b_iodone = reiserfs_end_buffer_io_sync ;
...
static int journal_read(struct super_block *p_s_sb) {
  ...
  */
  ...
  jh = (struct reiserfs_journal_header *)(SB_JOURNAL(p_s_sb)->j_header_bh->b_data) ;
  if ((int)(jh->j_first_unflushed_offset) >= 0 && 
...
static int reiserfs_journal_commit_thread(void *nullp) {
 ...
 init_waitqueue_head(&reiserfs_commit_thread_wait);
 init_waitqueue_head(&reiserfs_commit_thread_done);
#if 0
  daemonize() ;
