...
#endif
printf("fix_tail_page_for_writing\n");
}
...
static void init_inode (struct inode * inode, struct path * path)
{
    ...
    f = spin_lock(&keycopy_lock);
    copy_key (INODE_PKEY (inode), &(ih->ih_key));
    spin_unlock(&keycopy_lock, f);
...
out:
    ...
    return retval ;
#endif
printf("map_block_for_writepage???\n");
}
...
printf("reiserfs_write_full_page??\n");
#if 0
    if (!page->buffers) {
...
struct address_space_operations reiserfs_address_space_operations = {
    ...
    /*sync_page:* block_sync_page*/NULL,
