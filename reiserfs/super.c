...
struct super_operations reiserfs_sops = 
{
  ...
  /*write_super_lockfs: reiserfs_write_super_lockfs*/ NULL,
  /*unlockfs: reiserfs_unlockfs*/ NULL,
...
static int read_super_block (struct super_block * s, int size, int offset)
{
    ...
    */
    s->s_maxbytes = ((__s64)512 << 32) - s->s_blocksize ;
    return 0;
}
...
static int reread_meta_blocks(struct super_block *s) {
  int i ;
  mark_buffer_uptodate(SB_BUFFER_WITH_SB(s), 0);
  ...
  for (i = 0; i < SB_BMAP_NR(s) ; i++) {
    mark_buffer_uptodate(SB_AP_BITMAP(s)[i].bh, 0);
    ll_rw_block(READ, 1, &(SB_AP_BITMAP(s)[i].bh)) ;
    wait_on_buffer(SB_AP_BITMAP(s)[i].bh) ;
    if (!buffer_uptodate(SB_AP_BITMAP(s)[i].bh)) {
...
__u32 find_hash_out (struct super_block * s)
{
    ...
    inode = s->s_root;		/* s->s_root->d_inode; */
...
struct super_block * reiserfs_read_super (struct super_block * s, void * data, int silent)
{
   ...
   if ( get_hardblocksize(dev) < sb_block_count(rs)*(sb_blocksize(rs)>>10) ) {
