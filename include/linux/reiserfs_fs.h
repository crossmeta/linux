...
#define ROUND_UP(x) _ROUND_UP(x,8I64)
...
#define RASSERT( cond, format, args)		ASSERT(cond)
#if 0
if( !( cond ) ) 								\
...
(DEH_SIZE * 2 + ROUND_UP (1) + ROUND_UP (2))
...
#define GET_HASH_VALUE(offset) ((offset) & 0x7fffff80I64)
/* generation number occupies 7 bits starting from 0 up to 6 */
#define GET_GENERATION_NUMBER(offset) ((offset) & 0x7fI64)
...
#define journal_hash(t,dev,block) ((t)[_jhashfn((dev2udev(dev)),(block)) & JBH_HASH_MASK])
...
int FASTCALL buffer_journaled(const struct buffer_head *bh) ;
int FASTCALL mark_buffer_journal_new(struct buffer_head *bh) ;
...
/* stree.c */
int FASTCALL B_IS_IN_TREE(const struct buffer_head *);
					      ...
					      b_blocknr_t *new_blocknrs, int amount_needed)
{
    ...
    hint.th = tb->transaction_handle;
    hint.path = tb->tb_path;
    hint.inode = NULL;
    hint.key = tb->key;
    hint.block = 0;
    hint.formatted_node = 1;
